...
export default function Students({ onOpenAuth }) {
  ...
  const studentJumpInputRef = useRef(null);
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      // Fetch events
      try {
        const evData = await studentService.getEvents();
        setEvents(evData.events || []);
      } catch (err) {
        console.error("Failed to load events in Student portal", err);
      }
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex flex-col lg:flex-row">
        ...
        <main className="flex-1 p-6 lg:p-10 max-w-7xl mx-auto w-full">
          ...
          {/* --- TAB: EVENT CALENDAR --- */}
          {activeTab === "calendar" && (() => {
            const studentFilteredEvents = events.filter(evt => {
              const eventDept = evt.department || "";
              const studentDept = studentProfile.department || "";
              if (eventDept && studentDept && eventDept.toLowerCase() !== studentDept.toLowerCase()) {
                return false;
              }
              const eventSem = evt.semester || "";
              const studentSem = studentProfile.semester || "";
              if (eventSem && studentSem && eventSem.toLowerCase() !== studentSem.toLowerCase()) {
                return false;
              }
              return true;
            });
            ...
            const studentSortedEvents = [...studentFilteredEvents].sort((a, b) => {
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="relative flex items-center justify-center">
                            <button
                              type="button"
                              onClick={() => {
                                if (studentJumpInputRef.current) {
                                  try { studentJumpInputRef.current.showPicker(); } catch (err) { studentJumpInputRef.current.click(); }
                                }
                              }}
                      ...
                      <div className="grid grid-cols-7 gap-3 mt-3 text-sm font-semibold text-slate-700">
                        {(() => {
                          ...
                          for (let day = 1; day <= totalDays; day++) {
                            ...
                            const dayEvents = studentFilteredEvents.filter(evt => {
                ...
                <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200/50 lg:col-span-2 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="space-y-4">
                      {studentSortedEvents.length === 0 ? (
                        <div className="text-center py-12 text-slate-400 text-xs font-semibold">
                          No campus events scheduled.
                        </div>
                      ) : (
                        studentSortedEvents.map((evt, idx) => (
